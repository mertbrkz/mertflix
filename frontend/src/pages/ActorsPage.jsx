...
export default function ActorsPage() {
  ...
  }, [authNonce, activeQuery, langNonce])
  ...
  if (error) return <ErrorState title={t('actors_load_failed')} message={error} onRetry={() => setAuthNonce((n) => n + 1)} />

  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-6">
        <div>
          <h2 className="text-3xl font-bold">{t('actors_popular_title')}</h2>
          <div className="text-sm text-white/70 mt-1">{t('common_page')}: {uiPage}{hasMore ? t('common_more_available') : ''}</div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:justify-end">
          <input
            ...
            placeholder={t('actors_search_placeholder')}
