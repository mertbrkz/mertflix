...
export default function ActorDetailPage() {
  ...
  if (isTmdbMissing) {
    return (
      <div className="max-w-xl">
        ...
        <p className="text-white/70 mb-4">{t('listing_tmdb_required_body')}</p>
  ...
  if (error) return <ErrorState title={t('actor_load_failed')} message={error} onRetry={() => setAuthNonce((n) => n + 1)} />
    ...
    <div>
      <div className="flex flex-col md:flex-row gap-6 md:items-start mb-8">
        ...
        <div className="flex-1">
          ...
          {actor?.birthday && (
            <div className="text-white/60 text-sm mt-2">{t('actor_birth')}: {actor.birthday}{actor?.place_of_birth ? ` • ${actor.place_of_birth}` : ''}</div>
          )}
          {actor?.biography && (
      ...
      <div className="flex items-baseline justify-between mb-4">
        <h2 className="text-2xl font-bold">{t('actor_credits_title')}</h2>
        <div className="text-sm text-white/60">{credits.length} {t('common_results')}</div>
      </div>
        ...
        <div className="space-y-8">
          <section>
            <div className="flex items-baseline justify-between mb-3">
              <h3 className="text-xl font-bold">{t('actor_movies')}</h3>
            ...
            {movies.length ? (
              ...
              <p className="text-white/70">{t('actor_no_movies')}</p>
            )}
          </section>

          <section>
            <div className="flex items-baseline justify-between mb-3">
              <h3 className="text-xl font-bold">{t('actor_shows')}</h3>
            ...
            {shows.length ? (
              ...
              <p className="text-white/70">{t('actor_no_shows')}</p>
            )}
          </section>
        </div>
      ) : (
        <p className="text-white/70">{t('listing_no_results')}</p>
      )}

      {loading && <div className="text-center mt-4 text-white/70">{t('common_loading')}</div>}
    </div>
  )
}
